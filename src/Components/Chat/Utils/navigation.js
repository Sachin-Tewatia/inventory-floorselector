...
export const normalizeNavigationTarget = (target, projectId) => {
  ...
  const parts = input
    ...
    .split("/")
    .filter(
      (p) => p && p !== projectId && p !== "project" && p !== "salarpuria"
    );
  let towerId = null;
  let floorId = null;
  let unitId = null;
  ...
  if (parts.length === 2 && parts[0] === "floor" && parts[1].includes("_")) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (part === "tower" && parts[i + 1]) {
        ...
        part.startsWith("cluster") ||
        (part.startsWith("t") && /\d/.test(part))
      ) {
        towerId = part;
      }
    }
  ...
  if (towerId) {
    ...
    if (TOWER_MAP[towerId]) {
      ...
      // Extract number from tower ID
      const numMatch = towerId.match(/\d+/);
      if (numMatch) {
        const num = numMatch[0];
        // Convert "t8" or "8" to "cluster8"
        if (towerId.startsWith("t")) {
          towerId = `cluster${num}`;
        } else if (/^\d+$/.test(towerId)) {
          // Pure numeric tower ID (e.g., "8" -> "cluster8")
          towerId = `cluster${num}`;
        } else if (!towerId.startsWith("cluster")) {
          // If it has a number but doesn't start with "cluster", convert it
          towerId = `cluster${num}`;
        }
      }
    }
